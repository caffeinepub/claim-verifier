...
actor {
  ...
  func seedClaimsData() : [Claim] {
    ...
    seeds.map<{ title : Text; description : Text; category : Text }, Claim>(
      func(s) {
        { id = 0; title = s.title; description = s.description; category = s.category; sessionId = ""; timestamp = Time.now(); imageUrls = []; urls = [] }
      }
    )
  };
  ...
  public shared func createClaim(
    ...
    claimCount += 1;
    let claim : Claim = {
      id = claimCount; title; description; category; sessionId;
      timestamp = now; imageUrls; urls;
    };
  ...
  public shared func submitEvidence(
    ...
    let evidence : Evidence = {
      id = evidenceCount; claimId; sessionId; text;
      timestamp = now; imageUrls; urls;
    };
  ...
  public shared func reportContent(targetId : Nat, targetType : Text, sessionId : Text) : async { #ok; #err : Text } {
  ...
  public shared func restoreClaim(id : Nat, password : Text) : async { #ok; #err : Text } {
  ...
  public shared func restoreEvidence(id : Nat, password : Text) : async { #ok; #err : Text } {
    if (password != ADMIN_PASSWORD) { return #err("Unauthorized") };
    reportsArray := reportsArray.filter(func(r : Report) : Bool {
  ...
  public shared func adminDeleteClaim(id : Nat, password : Text) : async { #ok; #err : Text } {
    if (password != ADMIN_PASSWORD) { return #err("Unauthorized") };
    let evidenceIds = evidencesArray
      .filter(func(e : Evidence) : Bool { e.claimId == id })
      .map<Evidence, Nat>(func(e : Evidence) : Nat { e.id });
  ...
  public shared func adminDeleteEvidence(id : Nat, password : Text) : async { #ok; #err : Text } {
    if (password != ADMIN_PASSWORD) { return #err("Unauthorized") };
